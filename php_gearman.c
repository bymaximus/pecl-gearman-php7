...
PHP_FUNCTION(gearman_client_do_epoch) {
	...
	obj->ret= gearman_client_do_epoch(&(obj->client), 
					...
					(size_t)workload_len, job_handle->val);
...
static zend_function_entry gearman_client_methods[]= {
	...
	PHP_ME_MAPPING(doEpoch, gearman_client_do_epoch, arginfo_oo_gearman_client_do_epoch, ZEND_ACC_PUBLIC)
	...
	PHP_ME_MAPPING(jobExistsByUniqueKey, gearman_client_job_exists_by_unique_key, arginfo_oo_gearman_client_job_exists_by_unique_key, ZEND_ACC_PUBLIC)
